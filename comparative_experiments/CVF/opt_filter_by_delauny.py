...
import DPDERL
from comparative_experiments.CVF import sorted_cluster
...
from preprocessing.extract_filiter_point import cvf_extract_point
...
point_to_triangles = []  # 存储各中心点所连接的三角形
...
def read_clusters():
    ...
    #         cluster = [int(item) for item in oneline if item]  # 使用列表推导式并检查空字符串  V
...
def deal():
    ...
    while flag == 0:  # 如果每个簇未满足阈值要求则继续处理
        if len(clusters[d]) // 2 > (threshold + 1):
            date = datetime.now()
            print(f"#{cnt}: {date}:当前分析簇为{d} 簇内剩余点数为{len(clusters[d]) // 2 - 1}")
            cnt += 1
            # 簇中叶子节点的位置
            m = len(clusters[d]) - 2
            n = len(clusters[d]) - 1
            ...
            manage_clusters_cover(d, clusters[d])
            clusters_cover[d] -= current_viewshed
            ...
            while j <= len(clusters) - 1:
                ...
                manage_clusters_cover(l, clusters[l])
                all_cover_add = clusters_cover[l] + current_viewshed
                current_overlap = get_overlap(current_viewshed, all_cover_add, coverage)
                ...
                while k <= len(clusters[l]) - 1 and count < 5:
                    ...
                    temp_viewshed, _ = DPDERL.analysis_by_spderl_simplified(lon, lat, demdata.height[row][col],
                                                                            radius, h_stand)
                    temp_viewshed = transform_matrix(temp_viewshed, [row, col])
                    temp_coverage = int(np.sum(temp_viewshed))
                    temp_overlap = get_overlap(temp_viewshed, clusters_cover[l], temp_coverage)
...
def main():
    global point_to_triangles, clusters
    clusters = sorted_cluster.main()
    print("排序完成")
    ...
    # 写入文件
    input_file_path = f"../../output/csv/cvf_filtered_mountain_feature_{cvf_time}.csv"
    output_file_path = f'../../output/csv/cvf_filtered_clusters_data_point_{cvf_time}.txt'
    with open(input_file_path, 'w') as file:
        print("把筛选后的簇写入文本文件中")
        for cluster in clusters:
            file.write(",".join(map(str, cluster)) + "\n")
    print("写入文本结束")
    cvf_extract_point(input_file_path, output_file_path)
